...
var (
	rootCmd = &cobra.Command{
		Use:        "sysbench",
		Short:      " a load tester like sysbench",
		SuggestFor: []string{"sys bench to  test databases"},
	}
)
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&mysqlUser, "mysql-user", "root", "MySQL user(Default root)")
	rootCmd.PersistentFlags().StringVar(&mysqlPassword, "mysql-password", "123456", "MySQL password(Default 123456)")
	...
	rootCmd.PersistentFlags().StringVar(&sshUser, "ssh-user", "test", "SSH server user(Default test)")
	rootCmd.PersistentFlags().StringVar(&sshPassword, "ssh-password", "test", "SSH server password(Default test)")
